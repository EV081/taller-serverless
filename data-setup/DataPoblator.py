...
# Mapeo de archivos JSON -> tablas/keys
TABLE_MAPPING = {}
...
if TABLE_EMPLEADOS:
    TABLE_MAPPING["empleados.json"] = {
        ...
        "pk": "local_id",
        "sk": "correo"  # Actualizado: correo en lugar de dni
    }
...
def batch_write_items(table, items, table_name):
    """Escribe items en lotes con reintentos."""
    success_count = 0
    error_count = 0
            ...
            for future in as_completed(futures):
                try:
                    ...
                    with count_lock:
                        ...
                        if (success_count % 50 == 0) or (success_count + error_count >= total_items):
...
def populate_table(filename, table_config):
    ...
    if not table_name:
        print(f"   ⚠️  No hay nombre de tabla configurado para {filename}")
        return False
        ...
        if e.response['Error']['Code'] == 'ResourceNotFoundException':
            try:
                ...
                table_config = {
                    ...
                    'Tags': [
                        {'Key': 'Project', 'Value': 'burger-cloud'},
...
def create_all_resources():
    """Crea tablas DynamoDB necesarias."""
    print("\n" + "=" * 60)
    print("🏗️  CREANDO RECURSOS AWS (Burger Cloud)")
    print("=" * 60)
    ...
    # Empleados: PK = local_id, SK = correo (actualizado)
    if not create_dynamodb_table(
        table_name=TABLE_EMPLEADOS,
        key_schema=[
    ...
    # Pedidos: PK = local_id, SK = pedido_id con GSI by_usuario
    if not create_dynamodb_table(
...
def main():
    ...
    if not os.path.exists(DATA_DIR):
        ...
        print("   Ejecuta primero: python3 DataGenerator.py")
        return
